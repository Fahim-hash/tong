...
// HARD-CODED WHITE-LIST: Only user IDs matching this explicit array can initialize the control matrix
  ...
  useEffect(() => {
    const verifyAdministrativeClearance = async () => {
      ...
      try {
        ...
        const cleanedId = String(currentSession.id).trim().toLowerCase();
    ...
    <div className="min-h-screen bg-[#090d14] text-stone-200 pb-16 selection:bg-[#c1121f] selection:text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-stone-950 border border-stone-800 rounded-2xl p-6 shadow-2xl sticky top-24">
          ...
          <div className="space-y-3">
            {filteredMembers.length > 0 ? (
              filteredMembers.map((member) => (
                <div key={member.id} className="bg-stone-950 border border-stone-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-stone-700 transition">
                  <div className="flex items-center space-x-4">
